...
<script>
    export default {
        ...
        methods: {
            ...
            getFacultyUsers() {
                api.call('get','/api/admin/users/by-role/lecturer').then(response=>{
                    this.lecturers = response.data.data;
                });
            },
            addFacultyModal(classroom = null, store=false) {
                if( store ) {
                    api.call('post', '/api/admin/classroom-user/store', this.classroom_user).then(response=>{
                        if( response.status === 200 ) {
            ...
            getLearnerUsers() {
                api.call('get','/api/admin/users/by-role/learner').then(response=>{
