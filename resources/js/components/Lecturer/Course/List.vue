...
<script>
    export default {
        ...
        methods: {
            getCourses() {
                api.call('get', '/api/lecturer/courses').then(response=>{
            ...
            update(course_id) {
                ...
                api.call('post', `/api/lecturer/courses/update/${course_id}`, data).then(response=>{
                    if(response.status === 200) {
                        this.updating = false;
            ...
            destroy(course) {
                api.call('get', `/api/lecturer/courses/destroy/${course.id}`).then(response=>{
                    if(response.status === 200) {
                        this.$modal.hide('edit-modal');
                        this.getCourses();
